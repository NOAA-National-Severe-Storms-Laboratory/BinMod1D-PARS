...
def loss_kernel_numba(dN_out, dMi_out, dMj_out, C, x1, x2, y1, y2):
    """
...
def tri_kernel_numba(dN_out, dM_out, C, x1, y1, x2, y2, x3, y3, w, L):
    """
...
# 1. Enable Parallelism
@nb.jit(nopython=True, parallel=True, cache=False, fastmath=False)
def breakup_1mom_kernel(
    M_gain, M_loss, dMb_kernel, i_idx, j_idx, h_idx, k_limit_idx, indb, Ebr
):
    """
    2-moment breakup calculations using numba
    """
...
def combined_coeffs_tensor(f, params):
    ...
    C = np.zeros(
        (3, 3) + aki1.shape[:2] + (aki1.shape[2], aki2.shape[3]), dtype=np.float64
    )
...
def vectorized_2mom(params, w, L, dMb_kernel, dNb_kernel, indc, indb, dnum, Hlen, bins):
    """
    ...
    loss_kernel_numba(
        dNi_rect, dMi_rect, dMj_rect, Cr.reshape(9, -1), xr1, xr2, yr1, yr2
    )
    ...
    tri_kernel_numba(
        dN_gain_tri, dM_gain_tri, Ct.reshape(9, -1), xt1, yt1, xt2, yt2, xt3, yt3, w, L
    )
    ...
    k_gain = np.concatenate(
        (
            kmin_l[reg2],
            kmid_l[reg2],  # Region 2: kmin tri
            kmin_l[reg3],
            kmid_l[reg3],  # Region 3: kmin tri
            kmin_l[reg4],  # Region 4: kmin rect; self collection
            kmin_l[reg5],
            kmid_l[reg5],  # Region 5: kmin rect + tri
            kmin_l[reg6],
            kmid_l[reg6],  # Region 6: kmin rect + tri
            kmid_l[reg7],
            kmin_l[reg7],  # Region 7: kmid tri
            kmin_l[reg8],
            kmid_l[reg8],  # Region 8: kmin tri
            kmin_l[reg9],  # Region 9:kmin rect; fully in bin
            kmid_l[reg10],  # Region 10: kmid rect; fully in bin
            kmid_l[reg11],
            kmin_l[reg11],  # Region 11: kmid tri
            kmid_l[reg12],
            kmin_l[reg12],
        )
    )  # Region 12: kmid tri

    h_gain = np.concatenate(
        (
            hind_l[reg2],
            hind_l[reg2],  # Region 2: kmin tri
            hind_l[reg3],
            hind_l[reg3],  # Region 3: kmin tri
            hind_l[reg4],  # Region 4: kmin rect; self collection
            hind_l[reg5],
            hind_l[reg5],  # Region 5: kmin rect + tri
            hind_l[reg6],
            hind_l[reg6],  # Region 6: kmin rect + tri
            hind_l[reg7],
            hind_l[reg7],  # Region 7: kmid tri
            hind_l[reg8],
            hind_l[reg8],  # Region 8: kmin tri
            hind_l[reg9],  # Region 9:kmin rect; fully in bin
            hind_l[reg10],  # Region 10: kmid rect; fully in bin
            hind_l[reg11],
            hind_l[reg11],  # Region 11: kmid tri
            hind_l[reg12],
            hind_l[reg12],
        )
    )  # Region 12: kmid tri
...
def init_rk(rk_order):
    ...
    tableaux = {
        1: dict(a=[[0]], b=[1], c=[0]),  # RK 1st order
        2: dict(a=[[0, 0], [0.5, 0]], b=[0, 1], c=[0, 0.5]),  # RK 2nd order
        3: dict(
            a=[[0, 0, 0], [0.5, 0, 0], [-1, 2, 0]],  # RK 3rd order
            b=[1 / 6, 2 / 3, 1 / 6],
            c=[0, 0.5, 1],
        ),
        4: dict(
            a=[
                [0, 0, 0, 0],  # RK 4th order
                [0.5, 0, 0, 0],
                [0, 0.5, 0, 0],
                [0, 0, 1, 0],
            ],
    ...
    RK = {
        "a": np.array(tableaux[rk_order]["a"], dtype=float),
...
def combined_coeffs_array(f, ax, cx, ay, cy):
    ...
    # C11 * x * y
    C[1, 1, :] = d * cx * cy + b * cx * ay + c * ax * cy + a * ax * ay
...
def setup_regions(bins, kr, ir, jr, x11, x21, x12, x22, xk_min):
    ...
    check_bottom = (x11 < x_bottom_edge) & (x21 > x_bottom_edge)

    check_top = (x11 < x_top_edge) & (x21 > x_top_edge)

    check_left = (x12 < y_left_edge) & (x22 > y_left_edge)

    check_right = (x12 < y_right_edge) & (x22 > y_right_edge)
    ...
    cond_rect = (
        (~cond_touch) & (~cond_4) & (~cond_5) & (~cond_6) & (~cond_7) & (~cond_8)
    )
...
def calculate_rates(
    ...
    if len(k2) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k2], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k3) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k3], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k5) > 0:
        ...
        rect_000, rect_001 = rect_gain_integrals(
            C[:, :, k5], x1=xr1, x2=xr2, y1=yr1, y2=yr2
        )
        tri_000, tri_001 = tri_gain_integrals(
            C[:, :, k5], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k6) > 0:
        ...
        rect_000, rect_001 = rect_gain_integrals(
            C[:, :, k6], x1=xr1, x2=xr2, y1=yr1, y2=yr2
        )
        tri_000, tri_001 = tri_gain_integrals(
            C[:, :, k6], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k7) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k7], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k8) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k8], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k2b) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k2b], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
    ...
    if len(k3b) > 0:
        ...
        dN_gain_temp, dM_gain_temp = tri_gain_integrals(
            C[:, :, k3b], xt1, yt1, xt2, yt2, xt3, yt3, w, L
        )
...
def calculate_regions_batch(
    ...
    if len(k2) > 0:
        ...
        dM_gain[kr[k2], ir[k2], jr[k2], 1] = (
            dM_loss[kr[k2], ir[k2], jr[k2]] - dM_gain[kr[k2], ir[k2], jr[k2], 0]
        )
        dN_gain[kr[k2], ir[k2], jr[k2], 1] = (
            dNi_loss[kr[k2], ir[k2], jr[k2]] - dN_gain[kr[k2], ir[k2], jr[k2], 0]
        )
    ...
    if len(k3) > 0:
        ...
        dM_gain[kr[k3], ir[k3], jr[k3], 1] = (
            dM_loss[kr[k3], ir[k3], jr[k3]] - dM_gain[kr[k3], ir[k3], jr[k3], 0]
        )
        dN_gain[kr[k3], ir[k3], jr[k3], 1] = (
            dNi_loss[kr[k3], ir[k3], jr[k3]] - dN_gain[kr[k3], ir[k3], jr[k3], 0]
        )
    ...
    if len(k5) > 0:
        ...
        dM_gain[kr[k5], ir[k5], jr[k5], 1] = (
            dM_loss[kr[k5], ir[k5], jr[k5]] - dM_gain[kr[k5], ir[k5], jr[k5], 0]
        )

        dN_gain[kr[k5], ir[k5], jr[k5], 0] = rect_000 + tri_000

        dN_gain[kr[k5], ir[k5], jr[k5], 1] = (
            dNi_loss[kr[k5], ir[k5], jr[k5]] - dN_gain[kr[k5], ir[k5], jr[k5], 0]
        )
    ...
    if len(k6) > 0:
        ...
        dM_gain[kr[k6], ir[k6], jr[k6], 1] = (
            dM_loss[kr[k6], ir[k6], jr[k6]] - dM_gain[kr[k6], ir[k6], jr[k6], 0]
        )

        dN_gain[kr[k6], ir[k6], jr[k6], 0] = rect_000 + tri_000
        dN_gain[kr[k6], ir[k6], jr[k6], 1] = (
            dNi_loss[kr[k6], ir[k6], jr[k6]] - dN_gain[kr[k6], ir[k6], jr[k6], 0]
        )
    ...
    if len(k7) > 0:
        ...
        dM_gain[kr[k7], ir[k7], jr[k7], 0] = (
            dM_loss[kr[k7], ir[k7], jr[k7]] - dM_gain[kr[k7], ir[k7], jr[k7], 1]
        )

        dN_gain[kr[k7], ir[k7], jr[k7], 0] = (
            dNi_loss[kr[k7], ir[k7], jr[k7]] - dN_gain[kr[k7], ir[k7], jr[k7], 1]
        )
    ...
    if len(k8) > 0:
        ...
        dM_gain[kr[k8], ir[k8], jr[k8], 1] = (
            dM_loss[kr[k8], ir[k8], jr[k8]] - dM_gain[kr[k8], ir[k8], jr[k8], 0]
        )
        dN_gain[kr[k8], ir[k8], jr[k8], 1] = (
            dNi_loss[kr[k8], ir[k8], jr[k8]] - dN_gain[kr[k8], ir[k8], jr[k8], 0]
        )
    ...
    if len(k2b) > 0:
        ...
        dM_gain[kr[k2b], ir[k2b], jr[k2b], 0] = (
            dM_loss[kr[k2b], ir[k2b], jr[k2b]] - dM_gain[kr[k2b], ir[k2b], jr[k2b], 1]
        )
        dN_gain[kr[k2b], ir[k2b], jr[k2b], 0] = (
            dNi_loss[kr[k2b], ir[k2b], jr[k2b]] - dN_gain[kr[k2b], ir[k2b], jr[k2b], 1]
        )
    ...
    if len(k3b) > 0:
        ...
        dM_gain[kr[k3b], ir[k3b], jr[k3b], 0] = (
            dM_loss[kr[k3b], ir[k3b], jr[k3b]] - dM_gain[kr[k3b], ir[k3b], jr[k3b], 1]
        )
        dN_gain[kr[k3b], ir[k3b], jr[k3b], 0] = (
            dNi_loss[kr[k3b], ir[k3b], jr[k3b]] - dN_gain[kr[k3b], ir[k3b], jr[k3b], 1]
        )
...
def vectorized_1mom(
    cki, params, dMi, dMj, dMg, kmin, kmid, dMb_kernel, indc, indb, dnum, Hlen, bins
):
    """
...
def LGN_int(n, mu, sig2, x1, x2):
    ...
    if np.any(mask_center):
        prob_interval[mask_center] = 0.5 * (
            scip.erfc(z1[mask_center]) - scip.erfc(z2[mask_center])
        )
...
def gam_int(n, mu, Dm, x1, x2):
    ...
    I = (
        ...
        * scip.gamma(n + mu + 1)
        * (
            scip.gammainc(n + mu + 1.0, (mu + 4) * x2 / Dm)
            - scip.gammainc(n + mu + 1.0, (mu + 4) * x1 / Dm)
        )
    )
...
def In_int(n, c, x1, x2):
    ...
    if n == 0:
        ...
        I = (1.0 / c) * (np.exp(-c * x1) - np.exp(-c * x2))

    elif n == 1:

        I = (1.0 / c**2) * (
            np.exp(-c * x1) * (c * x1 + 1.0) - np.exp(-c * x2) * (c * x2 + 1.0)
        )
