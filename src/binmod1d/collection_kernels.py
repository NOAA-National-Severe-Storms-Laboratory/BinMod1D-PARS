...
def CKE(di1d, dj1d, vi1d, vj1d):
    ...
    di, dj, vi, vj = np.broadcast_arrays(di1d, dj1d, vi1d, vj1d)

    di_cgs = 0.1 * di  # convert to cm
    dj_cgs = 0.1 * dj
...
def Weber_number(di1d, dj1d, vi1d, vj1d):
    ...
    vi_cgs = 100.0 * vi  # convert to cm/s
    vj_cgs = 100.0 * vj

    # Ensure ds is the smaller diameter (if not already sorted)
    ds = np.minimum(di_cgs, dj_cgs)
...
def Straub_params(di1d, dj1d, vi1d, vj1d):
    ...
    CW = (
        0.1 * We * CKE(di, dj, vi, vj)
    )  # Units come out in ergs (i.e., 1e-7 Joules; 1 erg = 0.1 micro Joule), divide by 10 to get micro Joules

    We = np.maximum(We, 1e-12)
    CW = np.maximum(CW, 1e-12)

    dl = 0.1 * np.maximum(di, dj)  # cm
    ds = 0.1 * np.minimum(di, dj)  # cm
    ...
    CW_thresh1 = gamCW > 7.0  # Threshold in micro Joules

    D1_mean = 0.04  # cm
    dD_1 = 0.0125 * CW**0.5
    Var1 = dD_1**2 / 12.0  # cm^2
    sig2_1 = np.log((Var1 / (D1_mean**2)) + 1.0)

    N1 = np.zeros_like(di, dtype=np.float64)
    ...
    CW_thresh2 = CW >= 21.0
    D2_mean = 0.095  # cm
    dD_2 = np.zeros_like(di, np.float64)
    dD_2[CW_thresh2] = 0.007 * (CW[CW_thresh2] - 21.0)

    # mu2 = D2_mean
    sig2_2 = dD_2**2 / 12.0

    N2 = np.zeros_like(di, np.float64)
    N2[CW_thresh2] = 0.22 * (CW[CW_thresh2] - 21.0)
    ...
    """
    CW_thresh3 = (CW_thresh2) & (CW <= 46.0)
    D3_mean = 0.9 * ds
    dD_3 = 0.01 * (1.0 + 0.76 * CW**0.5)

    # mu3 = D3_mean
    sig2_3 = dD_3**2 / 12.0

    N3 = np.zeros_like(di, np.float64)
    N3[~CW_thresh2] = 1.0
    N3[CW_thresh3] = 0.04 * (46.0 - CW[CW_thresh3])
    ...
    # M31 = N1 * LGN_int(3.,mu1,sig2_1,dmin,dl)
    # M32 = N2 * GAU_int(3.,mu2,sig2_2,dmin,dl)
    # M33 = N3 * GAU_int(3.,mu3,sig2_3,dmin,dl)

    # # Residual mass (without prefactors; just need to map to original grid)
    # M_frag_total = M31+M32+M33
    # M_parent_total = dl**3+ds**3

    # mass_overshoot = M_frag_total>M_parent_total

    # scale_factor = np.ones_like(M_parent_total)

    # scale_factor[mass_overshoot] = M_parent_total[mass_overshoot]/M_frag_total[mass_overshoot]

    # N1 *= scale_factor
    # N2 *= scale_factor
    # N3 *= scale_factor

    # M34 = np.maximum(0.,M_parent_total-(M_frag_total*scale_factor))

    # NOTE: Returned dictionary of distribution parameters. Converted to mm.
    Straub_dict["dist1"] = {
        "muf": np.log(10.0 * D1_mean) - 0.5 * sig2_1,
        "sig2f": sig2_1,
        "N": N1,
    }

    Straub_dict["dist2"] = {"mu": 10.0 * D2_mean, "sig2": 100.0 * sig2_2, "N": N2}

    Straub_dict["dist3"] = {"mu": 10.0 * D3_mean, "sig2": 100.0 * sig2_3, "N": N3}
...
def hydro_kernel(vtx, vty, Ax, Ay):
    ...
    Kxy = (
        0.001
        * np.abs(vtx[:, None] - vty[None, :])
        * (np.sqrt(Ax[:, None]) + np.sqrt(Ay[None, :])) ** 2.0
    )
...
def long_kernel(di, dj, vi, vj, Ai, Aj):
    ...
    Exy[dj2d < 0.1] = np.maximum(
        4.5e4 * (dj2d[dj2d < 0.1] / 20) ** 2 * (1.0 - 3e-4 / (di2d[dj2d < 0.1] / 20)),
        1e-3,
    )
...
def hall_kernel(di, dj, vi, vj, Ai, Aj):
    ...
    Dcol_tab = (
        ...
        * 2.0
        * np.array(
            [10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 100.0, 150.0, 200.0, 300.0]
        )
    )  # Dcol in mm
...
def straub_efficiency(di, dj, vi, vj, Ai, Aj):
    ...
    # NOTE: Try using np.broadcast_arrays() here. Seems to work a lot better.
    ...
    # Ec = exp(-1.15 * We)
    Ec = np.clip(np.exp(-1.15 * We), 0.0, 1.0)

    large_cond = (di2d > 0.06) | (dj2d > 0.06)

    Kxy = hall_kernel(
        di, dj, vi, vj, Ai, Aj
    )  # Hall kernel has Ecol * (Es=1.0) * Kxy_hydro

    Kxy_hydro = hydro_kernel(vi, vj, Ai, Aj)

    Kxy[large_cond] = (
        Ec[large_cond] * Kxy_hydro[large_cond]
    )  # (Ecol=1.0)*Ecoal_straub * Kxy_hydro
