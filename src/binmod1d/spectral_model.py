...
class spectral_1d:
    ...
    def load_case(self, filename):
        ...
        with Dataset(filename, "r", format="NETCDF4") as file_nc:
            ...
            self.rhobins = 2 ** (1.0 / self.sbin)  # scaling param for mass bins
            # Boundary logic for Scenarios A, B, C
            self.bound_low = (2.0 + self.rhobins) / 3.0
            self.bound_high = (1.0 + 2.0 * self.rhobins) / 3.0
            ...
            for habit in self.habit_names:
                ...
                self.habit_params[habit] = (
                    file_nc.groups["Habits"].groups[habit].__dict__
                )
                ...
                if "gam_norm" in habit_vars:
                    self.habit_params[habit]["gam_norm"] = bool(
                        self.habit_params[habit]["gam_norm"]
                    )
    ...
    def setup_case(
        ...
        **kwargs : TYPE
        ...
        self.Ecb = (
            self.Eagg + self.Ebr
        )  # # Total Collision/Breakup efficiency for loss term
        self.ztop = ztop
        self.zbot = zbot
        self.tmax = tmax
        ...
        self.dz = dz
        t = np.arange(
            0, self.tmax + self.dt, step=self.dt
        ...
        for dd in range(self.dnum):
            ...
            if dd == 0:  # Original Bin distribution
                ...
                dists[dd] = dist(
                    ...
                    dist_var=dist_var,
                    kernel=kernel,
                    habit_dict=self.habit_dict[self.habit_names[dd]],
                    ptype=ptype,
                    Tc=Tc,
                    mom_num=moments,
                )
            ...
            dists[dd].dh = (
                self.dz / dists[dd].vt
            )  # Residence time for steady-state height calculations
            dists[dd].dt = self.dt * np.ones_like(
                self.t
            )  # Residence time for box model (i.e., equal to dt)
        ...
        if isinstance(frag_dist, str):
            ...
            raise ValueError(
                "frag_dict must be either a string or a dictionary. Strings must match defaults in habits.py module."
            )
        ...
        self.dists = (
            dists  # 3D array of distribution objects (dist_num x height x time)
        )
        ...
        self.out_text = lambda M, Mf: "Total Mass = {:.2f} g/m^3 | Total Mass Flux = {:.2f} g/(m^2*s)".format(
            np.sum(M), np.sum(Mf)
        )
    ...
    def setup_habits(self, habit_params, psd_params=None):
        ...
        for hl in range(self.dnum):
            ...
            if ("arho" and "brho" in list(self.habit_dict[habit_name_temp])) and (
                "am" and "bm" not in list(self.habit_dict[habit_name_temp])
            ):
                # Note: Assume equivolume diameter for now.
                self.habit_dict[habit_name_temp]["am"] = (
                    0.001 * (np.pi / 6.0) * self.habit_dict[habit_name_temp]["arho"]
                )
                self.habit_dict[habit_name_temp]["bm"] = (
                    3.0 - self.habit_dict[habit_name_temp]["brho"]
                )

            if ("arho" and "brho" not in list(self.habit_dict[habit_name_temp])) and (
                "am" and "bm" in list(self.habit_dict[habit_name_temp])
            ):
                # Note: Assume equivolume diameter for now.
                self.habit_dict[habit_name_temp]["arho"] = (
                    1000.0 * (6.0 / np.pi) * self.habit_dict[habit_name_temp]["am"]
                )
                self.habit_dict[habit_name_temp]["brho"] = (
                    3.0 - self.habit_dict[habit_name_temp]["bm"]
                )
    ...
    def setup_radar(self):
        ...
        for dd in range(self.dnum):
            self.angs[:, dd, :] = np.tile(
                angular_moments(self.sigma[dd]).T, (self.bins, 1)
            ).T
        ...
        if self.ptype == "rain":
            ...
            self.eps1 = (1 + 2 * (self.rho1 / self.rhoi) * Ki) / (
                1 - (self.rho1 / self.rhoi) * Ki
            )
            self.eps2 = (1 + 2 * (self.rho2 / self.rhoi) * Ki) / (
                1 - (self.rho2 / self.rhoi) * Ki
            )
    ...
    def calc_micro(self):
        ...
        for dd in range(self.dnum):
            ...
            M3[dd, :, :] = (
                am ** (-3.0 / bm)
                * (self.calc_moments(3.0 / bm, self.moments)).sum(axis=2)[dd, :, :]
            )
            M4[dd, :, :] = (
                am ** (-4.0 / bm)
                * (self.calc_moments(4.0 / bm, self.moments)).sum(axis=2)[dd, :, :]
            )
        ...
        self.Dm[M3 > 0.0] = M4[M3 > 0.0] / M3[M3 > 0.0]

        self.Dmtot = np.full_like(M3tot, np.nan)
        self.Dmtot[M3tot > 0.0] = M4tot[M3tot > 0.0] / M3tot[M3tot > 0.0]
        ...
        self.N[self.N == 0.0] = np.nan
        self.M[self.M == 0.0] = np.nan
        self.Rm[self.Rm == 0.0] = np.nan

        self.Ntot[self.Ntot == 0.0] = np.nan
        self.Mtot[self.Mtot == 0.0] = np.nan
        self.Rmtot[self.Rmtot == 0.0] = np.nan
    ...
    def calc_radar(self):
        ...
        self.Zh[zh_tot > ref_min] = 10.0 * np.log10(zh_tot[zh_tot > ref_min])
        self.Zv[zv_tot > ref_min] = 10.0 * np.log10(zv_tot[zv_tot > ref_min])
        self.Zdr[(zh_tot > ref_min) & (zv_tot > ref_min)] = 10.0 * np.log10(
            zh_tot[(zh_tot > ref_min) & (zv_tot > ref_min)]
            / zv_tot[(zh_tot > ref_min) & (zv_tot > ref_min)]
        )
        self.Kdp = kdp_tot
        self.Zhhvv[zhhvv_tot > ref_min] = 10.0 * np.log10(
            zhhvv_tot[zhhvv_tot > ref_min]
        )
        self.Rhohv[rhohv_denom > ref_min] = (
            zhhvv_tot[rhohv_denom > ref_min] / rhohv_denom[rhohv_denom > ref_min]
        )
        ...
        self.ZH[zh_full > ref_min] = 10.0 * np.log10(zh_full[zh_full > ref_min])
        self.ZV[zv_full > ref_min] = 10.0 * np.log10(zv_full[zv_full > ref_min])
        self.ZDR[(zh_full > ref_min) & (zv_full > ref_min)] = 10.0 * np.log10(
            zh_full[(zh_full > ref_min) & (zv_full > ref_min)]
            / zv_full[(zh_full > ref_min) & (zv_full > ref_min)]
        )
        self.KDP = kdp_full
        self.ZHHVV[zhhvv_full > ref_min] = 10.0 * np.log10(
            zhhvv_full[zhhvv_full > ref_min]
        )
        self.RHOHV[rhohv_denom_full > ref_min] = (
            zhhvv_full[rhohv_denom_full > ref_min]
            / rhohv_denom_full[rhohv_denom_full > ref_min]
        )
    ...
    def diagnose_subgrid(self):
        ...
        xi1_4D = np.swapaxes(
            np.tile(self.xi1, (self.dnum, self.Hlen, self.Tout_len, 1)), 2, 3
        )
        xi2_4D = np.swapaxes(
            np.tile(self.xi2, (self.dnum, self.Hlen, self.Tout_len, 1)), 2, 3
        )

        if self.moments == 2:  # Diagnose 2 moment linear distributions

            dx4D = np.swapaxes(
                np.tile(self.dxbins, (self.dnum, self.Hlen, self.Tout_len, 1)), 2, 3
            )

            self.aki, self.cki, self.x1, self.x2 = update_2mom(
                self.Mbins,
                self.Nbins,
                self.rhobins,
                self.bound_low,
    ...
    def calc_moments(self, r, moments=2):  # Units are g^n
        ...
        if moments == 2:
            return self.aki * Pn(r + 1, self.x1, self.x2) + self.cki * Pn(
                r, self.x1, self.x2
            )
        elif moments == 1:
    ...
    def write_netcdf(self, filename, save_radar=False):
        ...
        with Dataset(filename, "w", format="NETCDF4") as file_nc:
            ...
            if self.moments == 2:
                Nbins = file_nc.createVariable(
                    "Nbins", "f8", ("dists", "height", "bins", "time_out")
                )
                Nbins.units = "#"
    ...
    def plot_time_height(self, var="Z",ax=None,**kwargs):
        """
        ...
        cmap, levels, levels_ticks, clabel, labelpad, fontsize, slabel = get_cmap_vars(
            var
        )
        ...
        if ax_switch:
            ...
            cbar.set_label(
                clabel, usetex=True, rotation=270, fontsize=fontsize, labelpad=labelpad
            )
    ...
    def plot_moments_radar(self, ax=None, tind=-1, plot_habits=False, **kwargs):
        ...
        **kwargs : dict
        ...
        # If we don't have radar variables, then just calculate them here.
        if not hasattr(self, "ZH"):
            self.calc_radar()
        ...
        if latex_check():
            plt.rc("text", usetex=True)
        plt.rc("font", family="serif")
        plt.rc("xtick", labelsize=20) # Changed from 16
        plt.rc("ytick", labelsize=20) # Changed from 16
        ...
        if self.int_type == 2:  # Box model
            ...
            ax[0, 0].set_ylabel("Nt (1/L)", usetex=True, fontsize=bfont_size)
            ax[0, 1].set_ylabel("Dm (mm)", usetex=True, fontsize=bfont_size)
            ax[0, 2].set_ylabel(r"WC (g/m$^{3}$)", usetex=True, fontsize=bfont_size)
            ax[0, 3].set_ylabel("R (mm/hr)", usetex=True, fontsize=bfont_size)
            ax[1, 0].set_ylabel("Z (dBZ)", usetex=True, fontsize=bfont_size)
            ax[1, 1].set_ylabel("ZDR (dB)", usetex=True, fontsize=bfont_size)
            ax[1, 2].set_ylabel("Kdp (deg/km)", usetex=True, fontsize=bfont_size)
            ax[1, 3].set_ylabel(r"$\rho_{\mathrm{hv}}$", usetex=True, fontsize=42)

            ax[1, 0].set_xlabel("Time (sec)", fontsize=22, usetex=True)
            ax[1, 1].set_xlabel("Time (sec)", fontsize=22, usetex=True)
            ax[1, 2].set_xlabel("Time (sec)", fontsize=22, usetex=True)
            ax[1, 3].set_xlabel("Time (sec)", fontsize=22, usetex=True)

            ax_labelsize = 18
            ax[0, 0].axes.tick_params("both", labelsize=ax_labelsize)
            ax[0, 1].axes.tick_params("both", labelsize=ax_labelsize)
            ax[0, 2].axes.tick_params("both", labelsize=ax_labelsize)
            ax[0, 3].axes.tick_params("both", labelsize=ax_labelsize)
            ax[1, 0].axes.tick_params("both", labelsize=ax_labelsize)
            ax[1, 1].axes.tick_params("both", labelsize=ax_labelsize)
            ax[1, 2].axes.tick_params("both", labelsize=ax_labelsize)
            ax[1, 3].axes.tick_params("both", labelsize=ax_labelsize)
            ...
            ax[0, 0].plot(N_tot, self.z / 1000.0, label="total", **kw_pl_tot)
            ax[0, 1].plot(Dm_tot, self.z / 1000.0, **kw_pl_tot)
            ax[0, 2].plot(M_tot, self.z / 1000.0, **kw_pl_tot)
            ax[0, 3].plot(Rm_tot, self.z / 1000.0, **kw_pl_tot)

            ax[1, 0].plot(ZH, self.z / 1000.0, **kw_pl_tot)
            ax[1, 1].plot(ZDR, self.z / 1000.0, **kw_pl_tot)
            ax[1, 2].plot(KDP, self.z / 1000.0, **kw_pl_tot)
            ax[1, 3].plot(RHOHV, self.z / 1000.0, **kw_pl_tot)
            ...
            ax[0, 0].set_xlabel("Nt (1/L)", usetex=True, fontsize=x_font_size)
            ax[0, 1].set_xlabel("Dm (mm)", usetex=True, fontsize=x_font_size)
            ax[0, 2].set_xlabel(r"WC (g/m$^{3}$)", usetex=True, fontsize=x_font_size)
            ax[0, 3].set_xlabel("R (mm/hr)", usetex=True, fontsize=x_font_size)
            ax[1, 0].set_xlabel("Z (dBZ)", usetex=True, fontsize=x_font_size)
            ax[1, 1].set_xlabel("ZDR (dB)", usetex=True, fontsize=x_font_size)
            ax[1, 2].set_xlabel("Kdp (deg/km)", usetex=True, fontsize=x_font_size)
            ax[1, 3].set_xlabel(r"$\rho_{\mathrm{hv}}$", usetex=True, fontsize=36)


            y_font_size = 26
            ax[0, 0].set_ylabel("Height (km)", fontsize=y_font_size, usetex=True)
            ax[1, 0].set_ylabel("Height (km)", fontsize=y_font_size, usetex=True)
            ...
            if plot_habits:
                ...
                for d1 in range(dist_num):
                    ...
                    ax[0, 0].plot(N[d1, :], self.z / 1000.0, label=flabel, **kw_pl)
                    ax[0, 1].plot(Dm[d1, :], self.z / 1000.0, **kw_pl)
                    ax[0, 2].plot(M[d1, :], self.z / 1000.0, **kw_pl)
                    ax[0, 3].plot(Rm[d1, :], self.z / 1000.0, **kw_pl)
    ...
    def plot_dists(
        ...
        distscale="log",
        normbin=False,
        scott_solution=False,
        feingold_solution=False,
        plot_habits=False,
        plot_init=True,
        ax=None,
        **kwargs,
        ...
        **kwargs : dict
        ...
        if self.int_type == 0:
            f_label = "{:.1f} km | {:.2f} min.".format(
                self.z[hind] / 1000.0, self.tout[tind] / 60.0
            )

        elif self.int_type == 1:
            f_label = "{:.1f} km".format(self.z[hind] / 1000.0)

        elif self.int_type == 2:
            f_label = "{:.2f} min.".format(self.tout[tind] / 60.0)
        ...
        nN_init = (
            prefN_init
            * np.heaviside(mbins - xp1, 1)
            * np.heaviside(xp2 - mbins, 1)
            * (ap * mbins + cp)
        )
        nM_init = (
            prefM_init
        ...
        nN_final = (
            prefN_final
            * np.heaviside(mbins[None, :] - x1_final, 1)
            * np.heaviside(x2_final - mbins[None, :], 1)
            * (ak_final * mbins[None, :] + ck_final)
        )
        nM_final = (
            prefM_final
            * np.heaviside(mbins[None, :] - x1_final, 1)
        ...
        if scott_solution & (self.int_type == 2):
            ...
            self.n_scott = Scott_dists(
                self.xbins, 0.001 * self.Eagg, mu0 + 1, self.t, kernel_type=kernel_type
            )
            ...
            ax[1].plot(
                x, nm_scott_new, ":r", linewidth=2, label=f_label + " analytical"
            )
        ...
        if feingold_solution & (self.int_type == 2):
            ...
            if B > 0.0:
                ...
                if "lamf" in self.frag_dict:
                    ...
                    self.n_fein = Feingold_dists(
                        self.xbins, self.t, mu0 + 1, C, B, lamf, kernel_type=kernel_type
                    )
                ...
                if kernel_type == "SBE":
                    ...
                    ax[0].plot(
                        x, n_fein_new, ":r", linewidth=2, label=f_label + " analytical"
                    )
                    ax[1].plot(
                        x, nm_fein_new, ":r", linewidth=2, label=f_label + " analytical"
                    )
    ...
    def plot_dists_height(self, tind=-1, dz=1.0, plot_habits=False, **kwargs):
        """
        ...
        **kwargs : dict
        ...
        if ax is None:
            ax_switch = True
            fig, ax = plt.subplots(
                len(z_lvls), 1, figsize=(6, 10), sharey=True, sharex=True
            )
        else:
            ax_switch = False
        ...
        prefN_p = (
            self.dist0.am ** (1.0 / self.dist0.bm)
            * self.dist0.bm
            * mbins ** (1.0 - 1.0 / self.dist0.bm)
        )

        #!!! NOTE: Need to change? Size here.
        xbins = (mbins / self.dist0.am) ** (1.0 / self.dist0.bm)
        ...
        ax[0].set_ylabel(r"n(D) (1/m$^{3}$ 1/mm)", fontsize=20)
        ...
        for hh in range(1, len(z_lvls)):
            ...
            for d1 in range(self.dnum):
                ...
                prefN = am ** (1.0 / bm) * bm * mbins ** (1.0 - 1.0 / bm)

                nN_final[d1, :] = (
                    prefN
                    * np.heaviside(mbins - xp1_final, 1)
                    * np.heaviside(xp2_final - mbins, 1)
                    * (ap_final * mbins + cp_final)
                )
            ...
            ax[hh].set_ylabel(r"n(D) (1/m$^{3}$ 1/mm)", fontsize=20)
        ...
        for tt in range(1, self.Tlen):
            ...
            if pbar:
                pbar.set_description(
                    self.out_text(self.Ikernel.Mbins, self.Ikernel.Mfbins)
                )
                pbar.update(1)
            ...
            while progress < 1.0:
                ...
                if np.any(safe_mask_M):
                    ...
                    t_scale_M = (
                        0.5 * M_current[safe_mask_M] / np.abs(dM_dt[safe_mask_M])
                    )

                    # Fraction of TOTAL residence time
                    # Broadcast total_residence_time to match mask shape
                    res_time_masked = np.broadcast_to(
                        total_residence_time, M_current.shape
                    )[safe_mask_M]
                ...
                for ii in range(rklen):
                    if ii == 0:
                        ...
                        # Sum previous stages
                        k_sum_M = np.sum(
                            a[ii, :ii][None, None, None, :] * dM_stages[:, :, :, :ii],
                            axis=3,
                        )
                        M_stage = np.maximum(
                            M_current + current_step_vec * k_sum_M, 0.0
                        )
                ...
                M_current = np.maximum(M_current + current_step_vec * total_sum_M, 0.0)
            ...
            while dt_covered < dt_target:
                ...
                for ii in range(rklen):
                    if ii == 0:
                        ...
                        M_stage = np.maximum(M_current + step_size_arr * k_sum_M, 0.0)
                ...
                # Final Update
                total_sum_M = np.sum(b[None, None, None, :] * dM_stages, axis=3)
                M_current = np.maximum(M_current + step_size_arr * total_sum_M, 0.0)
        ...
        for tt in range(1, self.Tlen):
            ...
            while progress < 1.0:
                ...
                if np.any(safe_mask_N):
                    ...
                    # WORKING
                    t_scale_N = (
                        0.5 * N_current[safe_mask_N] / np.abs(dN_dt[safe_mask_N])
                    )

                    res_time_masked = np.broadcast_to(
                        total_residence_time, N_current.shape
                    )[safe_mask_N]
                    valid_res = res_time_masked > 1e-10
                    if np.any(valid_res):
                ...
                for ii in range(rklen):
                    if ii == 0:
                        ...
                        k_sum_N = np.sum(
                            a[ii, :ii][None, None, None, :] * dN_stages[:, :, :, :ii],
                            axis=3,
                        )
                        ...
                        N_stage = np.maximum(
                            N_current + current_step_vec * k_sum_N, 0.0
                        )
                ...
                M_current = np.maximum(M_current + current_step_vec * total_sum_M, 0.0)
                N_current = np.maximum(N_current + current_step_vec * total_sum_N, 0.0)
    ...
    def run_full_2mom(self, pbar=None):
        ...
        for tt in range(1, self.Tlen):
            ...
            while dt_covered < dt_target:
                ...
                rate_sed_M, rate_sed_N = self.get_sed_rate_2mom_only(
                    M_current, N_current
                )
                ...
                for ii in range(rklen):
                    if ii == 0:
                        ...
                        M_stage = np.maximum(M_current + step_arr_M * k_sum_M, 0.0)
                        ...
                        N_stage = np.maximum(N_current + step_arr_N * k_sum_N, 0.0)
                ...
                M_current = np.maximum(M_current + step_arr_M * total_sum_M, 0.0)
                N_current = np.maximum(N_current + step_arr_N * total_sum_N, 0.0)
    ...
    def run(self):
        ...
        try:
            ...
            if self.progress:
                ...
                with tqdm(
                    ...
                    miniters=None,
                    ncols=ncols,
                    desc="Running 1D spectral bin model",
                ) as pbar:
                    self._run_core(pbar)
        ...
        print(
            "Model Complete! Time Elapsed = {:.2f} min".format(
                (time_end - time_start).total_seconds() / 60.0
            )
        )
