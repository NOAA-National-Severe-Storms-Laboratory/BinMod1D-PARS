...
def Scott_moments(r, t, nu, E, B, kernel_type="Golovin"):
    ...
    if kernel_type == "Golovin":
        ...
        for tt in range(len(t)):
            gam_series[tt] = mpmath.nsum(
                lambda k: tau[tt] ** (k)
                * nu ** (nu * (k + 1))
                / ((tau[tt] + nu) ** (r + nu + k * (nu + 1)) * mpmath.factorial(k + 1))
                * (mpmath.gamma(nu + r + k * (nu + 1)) / mpmath.gamma(nu * (k + 1))),
                [0, np.inf],
                method="direct",
            )
        ...
        for tt in range(len(t)):
            M_rt[tt] = mpmath.nsum(
                ...
                * nu ** ((k + 1) * (nu + 1))
                * (T[tt] + nu) ** (-(nu + r + k * (nu + 2)))
                * mpmath.gamma(r + nu + k * (nu + 2))
                / (mpmath.factorial(k + 1) * mpmath.gamma((k + 1) * (nu + 1))),
                [0, np.inf],
                method="direct",
            )
...
def Feingold_moments(r, t, nu, B, gam, kernel_type="SBE"):
    ...
    if kernel_type == "SBE":
        ...
        for tt in range(len(t)):
            ...
            M_rt[tt] = (
                M0r
                + gam * (np.exp(B * gam * t[tt]) - 1) * gam ** (-1 - r) * gamma(1 + r)
            ) / den
        ...
        M_rt = (
            ...
            * (
                2
                * (gam - eta)
                * hyp2f1(0.5 * (r + 1), 0.5 * (r + 2), 1, (eta / (gam - eta)) ** 2)
                - eta
                * (r + 1)
                * np.sign(gam - eta)
                * hyp2f1(0.5 * (r + 2), 0.5 * (r + 3), 2, (eta / (gam - eta)) ** 2)
            )
        )
...
def Scott_dists(xbins, Eagg, nu, t, kernel_type="Golovin", max_k=100):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        npbins[:, 0] = (
            (1.0 / gamma(nu)) * (nu**nu) * xbins ** (nu - 1) * np.exp(-nu * xbins)
        )
    ...
    if kernel_type == "Golovin":
        ...
        if np.any(mask_sm):
            # Log Prefactor
            ln_pre = (
                np.log(tau + eps) + (nu - 1) * np.log(X_sm + eps) - (nu + tau) * X_sm
            )

            # Log Terms
            ln_coeffs = (
                (nu * (k + 1)) * np.log(nu) - gammaln(k + 2) - gammaln(nu * (k + 1))
            )
        ...
        if np.any(mask_sm):
            ...
            ln_powers = ((nu - 1) + k * (nu + 2)) * np.log(X_sm + eps) + k * np.log(
                T_vec + eps
            )
    ...
    if np.any(mask_lg):
        if kernel_type == "Golovin":
            tau = 1 - np.exp(-T_vec)
            num = (1 - tau) * np.exp(
                -(tau + nu) * X_lg + (nu + 1) * X_lg * tau ** (1.0 / (nu + 1))
            )
            den = (
                X_lg**1.5
                * tau ** ((2.0 * (nu - 1) + 3) / (2.0 * (nu - 1) + 4))
                * (2.0 * np.pi * (nu + 1) / nu) ** 0.5
            )
            corr = 1 - ((2.0 * (nu - 1) + 3) * (nu + 2)) / (
                24.0 * X_lg * tau ** (1 / (nu + 1)) * nu * (nu + 1)
            )
            ...
            den = (
                X_lg**2.5
                * (T_vec * (nu + 1) / nu) ** ((2.0 * nu + 3) / (2.0 * nu + 4))
                * (2.0 * np.pi * nu * (nu + 2)) ** 0.5
            )
            corr = 1 - ((nu + 3) * (2.0 * nu + 3)) / (
                24.0
                * X_lg
                * T_vec ** (1.0 / (nu + 2))
                * nu ** ((nu + 1) / (nu + 2))
            ...
            term1 = 0.9221 * 4.0 * nu * np.exp((ys - 1) * nu * X_lg)
            term2 = (T_vec + 2) ** 2 * (ys ** (nu - 1))
            term3 = (2.0 * np.pi * nu * (nu - (nu - 1) / (ys * X_lg))) ** 0.5
