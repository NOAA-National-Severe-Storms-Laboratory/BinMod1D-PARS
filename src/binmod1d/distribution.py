...
class dist:
    ...
    def __init__(
        ...
        if init_method == "gamma":
            ...
            Nt0 = habit_dict.get("Nt0", 1.0)  # #/m^3

            Mt0 = habit_dict.get("Mt0", 1.0)  # g/m^3
            Dm0 = habit_dict.get("Dm0", 2.0)  # mm
            mbar = habit_dict.get("mbar0", None)  # g
            mu0 = habit_dict.get("mu0", 3.0)
            ...
            self.bin_gamma_dist(
                Nt0=Nt0, Mt0=Mt0, mbar=mbar, mu0=mu0, Dm0=Dm0, normalize=gam_norm
            )
            ...
            func_nD = habit_dict.get("func_nD", lambda D: 1.0 * np.exp(-D / 1.0))
            ...
            if ("Mbins" in list(habit_dict.keys())) and (
                "Nbins" not in list(habit_dict.keys())
            ):

                self.Mbins = habit_dict.get("Mbins", np.zeros(self.bins))
                self.Nbins = self.Mbins / self.xbins

            if ("Mbins" not in list(habit_dict.keys())) and (
                "Nbins" in list(habit_dict.keys())
            ):
    ...
    def init_dist(
        ...
        if self.ptype == "rain":
            self.ar = habit_dict.get("ar", 1.0)
            self.br = habit_dict.get("br", 0.0)
            self.arho = habit_dict.get("arho", 1.0)
            self.brho = habit_dict.get("brho", 0.0)
            self.av = habit_dict.get("av", 3.78)  # Atlas and Ulbrich (1977)
            self.bv = habit_dict.get("bv", 0.67)  # Atlas and Ulbrich (1977)
            self.sigma = habit_dict.get("sig", 10.0)
            self.am = habit_dict.get(
                "am", 0.001 * (np.pi / 6.0)
            )  # Units: g * mm^(-(3+brho))
            self.bm = habit_dict.get("bm", 3.0)
            ...
            self.sigma = habit_dict.get("sig", 0.0)
            self.am = habit_dict.get(
                "am", 0.001 * (np.pi / 6.0) * habit_dict["arho"]
            )  # Units: g * mm^(-(3+brho))
            self.bm = habit_dict.get("bm", 3.0 - habit_dict["brho"])
        ...
        self.d = (self.xbins / self.am) ** (1.0 / self.bm)
        self.dmax = self.ar ** (-1.0 / 3.0) * self.d ** (1.0 - (self.br / 3.0))

        self.d_edges = (self.xedges / self.am) ** (1.0 / self.bm)
        self.dmax_edges = self.ar ** (-1.0 / 3.0) * self.d_edges ** (
            1.0 - (self.br / 3.0)
        )
        ...
        if ptype == "snow":
            ...
            self.rho = self.arho * self.d ** (self.bm - 3.0)
            self.rho[self.rho > rhoi] = rhoi

            self.rho1 = self.arho * self.d1 ** (self.bm - 3.0)
            self.rho1[self.rho1 > rhoi] = rhoi

            self.rho2 = self.arho * self.d2 ** (self.bm - 3.0)
        ...
        self.vt[self.vt > 10.0] = 10.0
        self.vt_edges[self.vt_edges > 10.0] = 10.0
        ...
        self.A = 0.25 * np.pi * self.d**2.0
        # Edge Area (mm^2)
        self.A_edges = 0.25 * np.pi * self.d_edges**2.0
    ...
    def bin_gamma_dist(
        self, Nt0=1.0, Mt0=1.0, mbar=None, mu0=3, Dm0=2, normalize=False
    ):
        """
        ...
        if (
            normalize
        ):  # Normalize mass distribution similar to Scott (1967) and Long (1974)
            ...
            nx_func = (
                lambda x: (self.Nt0 / self.bm)
                * (1.0 / scip.gamma(self.mu0 + 1.0))
                * (1.0 / self.mn)
                * (x / self.mn) ** ((nu / self.bm) - 1.0)
                * np.exp(-((x / self.mn) ** (1.0 / self.bm)))
            )
    ...
    def bin_direct_dist(self, edges, Nbins, Mbins=None, var="size"):
        ...
        interp_N = interp1d(
            edges, cdf_N, kind="linear", bounds_error=False, fill_value=(0, cdf_N[-1])
        )
    ...
    def diagnose_1mom(self):
        ...
        self.vtm[self.vtm > 10.0] = 10.0
        self.vtn[self.vtn > 10.0] = 10.0
    ...
    def diagnose(self):
        ...
        self.Mbins[self.Mbins < eps] = 0.0
        self.Nbins[self.Nbins < eps] = 0.0
        ...
        xm1 = xm / self.xi1
        cond_null = (self.Mbins <= 0.0) | (self.Nbins <= 0.0)

        # Boundary logic for Scenarios A, B, C
        bound_low = (2.0 + self.rhobins) / 3.0
        bound_high = (1.0 + 2.0 * self.rhobins) / 3.0

        cond_a = (bound_low <= xm1) & (xm1 <= bound_high) & (~cond_null)
        cond_b = (1.0 <= xm1) & (xm1 < bound_low) & (~cond_null)
        ...
        n1i[cond_a] = (
            2
            * (
                self.Nbins[cond_a] * (self.xi1[cond_a] + 2.0 * self.xi2[cond_a])
                - 3.0 * self.Mbins[cond_a]
            )
            / (dx[cond_a] ** 2)
        )
        n2i[cond_a] = (
            2
            * (
                -self.Nbins[cond_a] * (2.0 * self.xi1[cond_a] + self.xi2[cond_a])
                + 3.0 * self.Mbins[cond_a]
            )
            / (dx[cond_a] ** 2)
        )
        ...
        x2i[cond_b] = self.xi1[cond_b] + 3.0 * (xm[cond_b] - self.xi1[cond_b])
        n1i[cond_b] = (
            2.0
            * self.Nbins[cond_b]
            / (3.0 * np.maximum(eps, xm[cond_b] - self.xi1[cond_b]))
        )
        n2i[cond_b] = 0.0

        # Scenario C: Truncated at the left
        x1i[cond_c] = self.xi2[cond_c] - 3.0 * (self.xi2[cond_c] - xm[cond_c])
        n1i[cond_c] = 0.0
        n2i[cond_c] = (
            2.0
            * self.Nbins[cond_c]
            / (3.0 * np.maximum(eps, self.xi2[cond_c] - xm[cond_c]))
        )
        ...
        self.cki[valid] = (
            n1i[valid] * x2i[valid] - x1i[valid] * n2i[valid]
        ) / np.maximum(eps, local_dx[valid])
    ...
    def radar_bins(self):
        ...
        self.zh = 1000.0 * (ak_zh * self.moments(1.0) + ck_zh * self.moments(0.0))
        self.zv = 1000.0 * (ak_zv * self.moments(1.0) + ck_zv * self.moments(0.0))
        self.kdp = 1000.0 * (ak_kdp * self.moments(1.0) + ck_kdp * self.moments(0.0))
        self.zhhvv = 1000.0 * (
            ak_zhhvv * self.moments(1.0) + ck_zhhvv * self.moments(0.0)
        )

        self.Zh = -35.0 * np.ones_like(self.zh)
        self.Zv = -35.0 * np.ones_like(self.zv)
        self.Zdr = np.zeros_like(self.zh)
        self.Kdp = np.zeros_like(self.kdp)

        rad_fill = (self.zh > 0.0) & (self.zv > 0.0)

        self.Zh[rad_fill] = 10.0 * np.log10(self.zh[rad_fill])
        self.Zdr[rad_fill] = 10.0 * np.log10(self.zh[rad_fill] / self.zv[rad_fill])

        zh_sum = np.nansum(self.zh)
        zv_sum = np.nansum(self.zv)
        zhhvv_sum = np.abs(np.nansum(self.zhhvv))

        if (zh_sum > 0.0) & (zv_sum > 0.0):
            self.ZH = 10.0 * np.log10(zh_sum)
            self.ZDR = 10.0 * np.log10(zh_sum / zv_sum)
            self.KDP = np.nansum(self.kdp)
            rhohv_denom = np.sqrt(zh_sum * zv_sum)
            if (
                rhohv_denom > 0.0
            ):  # Apparently for very small zh_sum and zv_sum the denominator can still be zero.
    ...
    def plot(self, log_switch=True, x_axis="mass", ax=None):
        ...
        ax[1].plot(np.log10(xbins), 1000.0 * mbins**2 * n_init, "k")
...
def update_2mom(Mbins, Nbins, rhobins, bound_low, bound_high, dx, xi1, xi2):
    ...
    n1i[cond_a] = (
        2
        * (Nbins[cond_a] * (xi1[cond_a] + 2.0 * xi2[cond_a]) - 3.0 * Mbins[cond_a])
        / (dx[cond_a] ** 2)
    )
    n2i[cond_a] = (
        2
        * (-Nbins[cond_a] * (2.0 * xi1[cond_a] + xi2[cond_a]) + 3.0 * Mbins[cond_a])
        / (dx[cond_a] ** 2)
    )

    # Scenario B Logic
    x2i[cond_b] = xi1[cond_b] + 3.0 * (xm[cond_b] - xi1[cond_b])
    n1i[cond_b] = (
        2.0 * Nbins[cond_b] / (3.0 * np.maximum(eps, xm[cond_b] - xi1[cond_b]))
    )
    n2i[cond_b] = 0.0

    # Scenario C Logic
    x1i[cond_c] = xi2[cond_c] - 3.0 * (xi2[cond_c] - xm[cond_c])
    n2i[cond_c] = (
        2.0 * Nbins[cond_c] / (3.0 * np.maximum(eps, xi2[cond_c] - xm[cond_c]))
    )
    n1i[cond_c] = 0.0
...
def spheroid_factors(ar):
    ...
    La[ar < 1.0] = ((1 + kap[ar < 1.0] ** 2) / kap[ar < 1.0] ** 2) * (
        1 - np.arctan(kap[ar < 1.0]) / kap[ar < 1.0]
    )
    La[ar > 1.0] = ((1 - kap[ar > 1.0] ** 2) / kap[ar > 1.0]) * (
        (1 / (2 * kap[ar > 1.0])) * np.log((1 + kap[ar > 1.0]) / (1 - kap[ar > 1.0]))
        - 1
    )
    Lc = (1.0 - La) / 2.0
...
def angular_moments(sigma):
    ...
    ang4 = (0.375 - 0.5 * uu + 0.125 * uu**4) * (0.375 + 0.5 * uu + 0.125 * uu**4)
...
def dielectric_water(t, eps_0, t0=273.15, wave=110.0):
    """
    ...
    ew_real = ew_eps_inf + (
        (
            ...
            * (
                1
                + (ew_lambda / (0.001 * wave)) ** (1 - ew_alpha)
                * np.sin(ew_alpha * np.pi / 2)
            )
        )
        / (
            1
            ...
            * (ew_lambda / (0.001 * wave)) ** (1 - ew_alpha)
            * np.sin(ew_alpha * np.pi / 2)
            + (ew_lambda / (0.001 * wave)) ** (2 * (1 - ew_alpha))
        )
    )
    ew_imag = (
        (ew_eps_s - ew_eps_inf)
...
def extended_brandes(d):
    ...
    mask1 = d < d_crit
    res[mask1] = p_max + (1.0 - p_max) * (
        1.0 - np.sin((np.pi / 2) * (d[mask1] / d_crit))
    )
...
def rain_terminal_velocity(d_mm):
    ...
    return 9.43 * (
        1.0 - np.exp(-((0.565 * d_mm) ** (1.147)))
    )  # Equation 5 from Best (1950) https://doi.org/10.1002/qj.49707632905
