...
class Interaction:
    ...
    def __init__(
        ...
        self.ind_i, self.ind_j = np.meshgrid(
            np.arange(0, self.bins, 1), np.arange(0, self.bins, 1), indexing="ij"
        )
        ...
        self.Eagg = Eagg * np.ones(
            (self.pnum, Hlen, self.bins, self.bins), dtype=np.float64
        )
        self.Ebr = Ebr * np.ones(
            (self.pnum, Hlen, self.bins, self.bins), dtype=np.float64
        )
        ...
        if self.mom_num == 2:
            self.cond_1 = np.tile(
                (
                    (self.ind_i >= (self.bins - self.sbin))
                    | (self.ind_j >= (self.bins - self.sbin))
                ),
                (self.Hlen, 1, 1),
            )
            # WORKING
            self.self_col = np.ones(
                (self.Hlen, self.pnum, self.bins, self.bins), dtype=np.uint8
            )
            ...
            for d1 in range(self.dnum):
                for d2 in range(d1, self.dnum):
                    if d1 == d2:
                        # WORKING
                        self.self_col[:, dd, :, :] = np.tile(
                            np.triu(
                                np.ones((self.bins, self.bins), dtype=np.uint8), k=0
                            ),
                            (self.Hlen, 1, 1),
                        )
        ...
        self.static_mask = (
            (~(self.self_col.astype(bool))) | (self.cond_1[:, None, :, :])
        ).transpose(1, 0, 2, 3)
        ...
        if Ebr > 0.0:  # Setup fragment distribution if Ebr>0.
            self.setup_fragments()
        else:
            self.dMb_gain_kernel = np.zeros(
                (self.pnum, self.bins, self.bins, self.bins)
            )
            self.dNb_gain_kernel = np.zeros(
                (self.pnum, self.bins, self.bins, self.bins)
            )
    ...
    def setup_1mom(self):
        ...
        for d1 in range(self.dnum):
            for d2 in range(d1, self.dnum):
                ...
                # NEW METHOD
                region_inds, x_bottom_edge, x_top_edge, y_left_edge, y_right_edge = (
                    setup_regions(self.bins, kr, ir, jr, x11, x21, x12, x22, xk_min)
                )

                dMi_loss, dMj_loss, dM_loss, dM_gain, _, _ = calculate_rates(
                    self.Hlen,
                    self.bins,
                    region_inds,
                    x_bottom_edge,
    ...
    def get_dynamic_params_1mom(self):
        ...
        M_check = (self.Mbins[p1][:, :, :, None] == 0.0) | (
            self.Mbins[p2][:, :, None, :] == 0.0
        )
    ...
    def get_dynamic_params(self):
        ...
        cond2 = (
            (np.eye(bins, k=1, dtype=bool)[None, None, :, :])
            & c_BR
            & check_l
            & cond_touch
        )
        ...
        PK = np.broadcast_to(
            self.PK[:, :, None, :, :], (4, self.pnum, self.Hlen, self.bins, self.bins)
        ).reshape(4, -1)[:, active_inds]
        aki1 = np.broadcast_to(self.aki[p1][:, :, :, None], target_shape).ravel()[
            active_inds
        ]
        cki1 = np.broadcast_to(self.cki[p1][:, :, :, None], target_shape).ravel()[
            active_inds
        ]
        aki2 = np.broadcast_to(self.aki[p2][:, :, None, :], target_shape).ravel()[
            active_inds
        ]
        cki2 = np.broadcast_to(self.cki[p2][:, :, None, :], target_shape).ravel()[
            active_inds
        ]
    ...
    def create_kernels(self, dists):
        ...
        if (self.kernel == "Hydro") | (self.kernel == "Long") | (self.kernel == "Hall"):
            ...
            for d1 in range(self.dnum):
                for d2 in range(d1, self.dnum):
                    ...
                    if self.kernel == "Hydro":
                        HK[0, dd, :, :] = hydro_kernel(
                            dist1.vt1, dist2.vt1, dist1.A1, dist2.A1
                        )
                        HK[1, dd, :, :] = hydro_kernel(
                            dist1.vt2, dist2.vt1, dist1.A2, dist2.A1
                        )
                        HK[2, dd, :, :] = hydro_kernel(
                            dist1.vt1, dist2.vt2, dist1.A1, dist2.A2
                        )
                        HK[3, dd, :, :] = hydro_kernel(
                            dist1.vt2, dist2.vt2, dist1.A2, dist2.A2
                        )

                    elif self.kernel == "Long":
                        HK[0, dd, :, :] = long_kernel(
                            dist1.d1, dist2.d1, dist1.vt1, dist2.vt1, dist1.A1, dist2.A1
                        )
                        HK[1, dd, :, :] = long_kernel(
                            dist1.d2, dist2.d1, dist1.vt2, dist2.vt1, dist1.A2, dist2.A1
                        )
                        HK[2, dd, :, :] = long_kernel(
                            dist1.d1, dist2.d2, dist1.vt1, dist2.vt2, dist1.A1, dist2.A2
                        )
                        HK[3, dd, :, :] = long_kernel(
                            dist1.d2, dist2.d2, dist1.vt2, dist2.vt2, dist1.A2, dist2.A2
                        )
            ...
            PK[1, :, :, :] = (
                (HK[1, :, :, :] - HK[0, :, :, :]) / dist1.dxbins[None, :, None]
            ) - PK[3, :, :, :] * dist2.xi1[None, None, :]
            PK[2, :, :, :] = (
                (HK[2, :, :, :] - HK[0, :, :, :]) / dist2.dxbins[None, None, :]
    ...
    def calc_smootherstep(self, d_array, d_start, d_end):
        ...
        t = (d_array[mask_ramp] - d_start[mask_ramp]) / (
            d_end[mask_ramp] - d_start[mask_ramp]
        )
    ...
    def setup_fragments(self):
        ...
        if (d_start is not None) and (d_end is not None):
            ...
            scale_factor = (E_left + 4.0 * E_cen + E_right) / 6.0
        else:
        ...
        if var_name == "mass":
            ...
            Nb_raw = IF_func(0.0, child, p_i, p_j)
        ...
        if pbounds is not None:
            ...
            ratio_ramp = self.calc_smootherstep(
                d_ratio,
                0.1 * np.ones_like(max_parent_d),
                0.25 * np.ones_like(max_parent_d),
            )
            ...
            base_ramp = self.calc_smootherstep(
                max_parent_d,
                pbounds[0] * np.ones_like(max_parent_d),
                pbounds[1] * np.ones_like(max_parent_d),
            )
        ...
        scale_factor = np.where(surv_pairs, scale_factor, 0.0)
        ...
        self.Ebr = np.where(surv_pairs, self.Ebr, 0.0)
    ...
    def __plot_source_target(self, d1, d2, rtype, rind, invert=False, full=False):
        ...
        Mcheck = ((self.Mbins[d1, :, :] == 0.0)[:, :, None]) | (
            (self.Mbins[d2, :, :] == 0.0)[:, None, :]
        )  # If M1 or M2 is zero, do not include in bin-pair list.

        cond_1 = (
            self.cond_1 | Mcheck
        )  # New cond_1. Basically exclude bin-pairs that are off grid and ones involving empty bins.
        ...
        dMi_loss, dMj_loss, dM_gain, dNi_loss, dN_gain = calculate_regions_batch(
            self.Hlen,
            self.bins,
            kr,
            ir,
        ...
        M1_loss = np.nansum(
            dMi_loss, axis=2
        )  # Loss of dist1 mass with collisions from dist2
        N1_loss = np.nansum(
            dNi_loss, axis=2
        )  # Loss of dist1 number with collisions from dist2

        M2_loss = np.nansum(
            dMj_loss, axis=1
        )  # Loss of dist2 mass with collisions from dist1
        N2_loss = np.nansum(
            dNi_loss, axis=1
        )  # Loss of dist2 number with collisions from dist1
        ...
        np.add.at(
            M_gain,
            (np.arange(self.Hlen)[:, None, None], self.kmin),
            dM_gain[:, :, :, 0],
        )
        np.add.at(
            M_gain,
            (np.arange(self.Hlen)[:, None, None], self.kmid),
            dM_gain[:, :, :, 1],
        )
        ...
        print(
            "Total Region Number = {} out of {}".format(
                len(k2)
                + len(k3)
                + len(k4)
                + len(k5)
        ...
        if invert:
            ...
            if rtype == 2:
                ...
                kgain_t = Polygon(
                    ((xi1, xj1), (xi1, y_left_edge[ind]), (xi2, xj1)),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 3:
                kgain_t = Polygon(
                    (((xi1, xj1), (xi1, xj2), (x_bottom_edge[ind], xj1))),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 11:
                # Gain integral
                kgain_t = Polygon(
                    (((xi2, xj1), (x_top_edge[ind], xj2), (xi2, xj2))),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 12:
                kgain_t = Polygon(
                    ((xi1, xj2), (xi2, xj2), (xi2, y_right_edge[ind])),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 7:
                kgain_t = Polygon(
                    ((x_top_edge[ind], xj2), (xi2, xj2), (xi2, y_right_edge[ind])),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 8:
                kgain_t = Polygon(
                    ((xi1, xj1), (xi1, y_left_edge[ind]), (x_bottom_edge[ind], xj1)),
                    closed=True,
                    facecolor="purple",
                )
            elif np.isin(rtype, [4, 9, 10]):
                kgain_r = Polygon(
                    ((xi1, xj1), (xi1, xj2), (xi2, xj2), (xi2, xj1)),
                    closed=True,
                    facecolor="purple",
                )
            elif rtype == 5:
                ...
                kgain_t = Polygon(
                    (
                        (xi1, y_right_edge[ind]),
                        (xi1, y_left_edge[ind]),
                        (xi2, y_right_edge[ind]),
                    ),
                    closed=True,
                    facecolor="purple",
                )
            ...
            ax.plot(
                [dist1.xi1[ii], dist1.xi2[ii]],
                [kbin_min[ii, jj] - dist1.xi1[ii], kbin_min[ii, jj] - dist1.xi2[ii]],
                "b",
            )
            ax.plot(
                [dist1.xi1[ii], dist1.xi2[ii]],
                [xk_min[ind] - dist1.xi1[ii], xk_min[ind] - dist1.xi2[ii]],
                "k",
            )
            ax.plot(
                [dist1.xi1[ii], dist1.xi2[ii]],
                [kbin_max[ii, jj] - dist1.xi1[ii], kbin_max[ii, jj] - dist1.xi2[ii]],
                "r",
            )
    ...
    def interact_2mom(self, dt):
        ...
        M_loss, M_gain, N_loss, N_gain = vectorized_2mom(
            ...
            self.w,
            self.L,
            self.dMb_gain_kernel,
            self.dNb_gain_kernel,
            self.indc,
            self.indb,
            self.dnum,
            self.Hlen,
