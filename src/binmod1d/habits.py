...
def fragments(dist='exp',**kwargs):
    ...
    **kwargs : dict
    ...
    if dist=='exp':
        ...
        parent_cutoff = kwargs.pop('parent_cutoff',0.)
        ...
        if isinstance(cdf_bounds,tuple):
            d_start = np.exp(muf+np.sqrt(2*sig2f)*erfinv(2.*cdf_bounds[0]-1))
            d_end = np.exp(muf+np.sqrt(2*sig2f)*erfinv(2.*cdf_bounds[1]-1))
...
def straub_wrapper(n, c, pi, pj, cdf_bounds=None,state=None):
    ...
    mass_overshoot = M_frag_total > ms # Fragment total can't exceed the smaller i,j mass.
    ...
    x_res = np.maximum(0.,M_parent_total-M_frag_scaled)
